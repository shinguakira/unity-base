...
```powershell
# Editor を開く
powershell -ExecutionPolicy Bypass -File unity.ps1

# バッチモードでコンパイルチェック (Editor は自動で停止される)
powershell -ExecutionPolicy Bypass -File unity.ps1 compile
```

コンパイルログは `E:\tmp\unity-build.log` に出力されます。

## 動作環境

- Windows + Unity 2022.3 LTS
- 外部パッケージ不要
